...
```bash
cd /Users/nguyenhoangduong/Documents/webphim-java/Backe/DotnetBackend
docker exec -i webphim-mysql mysql -uroot -pwebphim123 webphim < final-fixed-test-data.sql
```

Nếu muốn reset database hoàn toàn bằng file dump đầy đủ:

```bash
docker exec -i webphim-mysql mysql -uroot -pwebphim123 webphim < Dump20260419.sql
```
